...
**Using Docker Compose (recommended)** — loads all build args from `apps/web/.env`:

```bash
docker compose --env-file apps/web/.env build web
```

**Using docker build** — pass each variable manually:
...
Build args are required because `generateStaticParams` connects to the database at build time.

### Run the container

**Using Docker Compose** (loads `apps/web/.env`):

```bash
docker compose --env-file apps/web/.env up web
```

**Using docker run** with an env file:

```bash
docker run -p 8080:8080 --env-file apps/web/.env firstroad-web
```

Or pass variables explicitly — add `INNGEST_SIGNING_KEY` and `INNGEST_EVENT_KEY` for production.
...
The Dockerfile uses the **official oven/bun** image for the build stage.
...
2. Use official oven/bun image for build
